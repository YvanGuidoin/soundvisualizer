...
class SoundVisualizer extends React.Component<SoundVisualizerProps> {
  ...
  private drawLogo(
    ...
    if (sizeToFill < 1) {
      ...
      canvasCtx.lineTo(
        midX + cosinusBegin * micCircleRadius,
        midY + sinusArc * micCircleRadius
      );
      canvasCtx.closePath();
      canvasCtx.fill();
      ...
      const sinusArc =
        (micCircleRadius - (sizeToFill - micCircleRadius * 2)) /
        micCircleRadius;
      const angle = Math.asin(sinusArc);
      const cosinusBegin = Math.cos(angle);
      canvasCtx.beginPath();
      ...
      canvasCtx.lineTo(
        midX + cosinusBegin * micCircleRadius,
        midY - micCircleRadius - sinusArc * micCircleRadius
      );
      canvasCtx.arc(
        midX,
        midY - micCircleRadius,
  ...
  componentWillReceiveProps(newProps: SoundVisualizerProps) {
    if (
      newProps.forceUpdater &&
      newProps.forceUpdater !== this.props.forceUpdater
    ) {
