...
class SoundVisualizer extends React.Component {
    ...
    static getRadiusFromProps(props) {
        return Math.min(props.width, props.height) * props.ratio / 2;
    }
    static getMaxBarSize(props) {
    ...
    draw() {
        ...
        if (this.analyser && this.analyser.numberOfInputs > 0) {
            ...
            /*         drawing         */
            let barHeight, x2, y2, overallVolumeSum = 0;
    ...
    drawLogo(canvasCtx, percentage) {
        ...
        if (sizeToFill < 1) {
            ...
            canvasCtx.lineTo(midX + cosinusBegin * smallCircleRadius, midY - smallCircleRadius - sinusArc * smallCircleRadius);
    ...
    render() {
        return React.createElement("canvas", { width: this.props.width, height: this.props.height, ref: this.canvas });
    }
}
